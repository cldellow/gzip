...
class GZIPInputStream extends InflaterInputStream {
     ...
     */
    private GZIPInputStream(CountingInputStream in, Inflater inflater, int size, MemberOffset cb) throws Exception {
        super(in, inflater, size);
        cis = in;
        try {
            Field field = InflaterInputStream.class.getDeclaredField("usesDefaultInflater");
            field.setAccessible(true);
            field.setBoolean(this, true);
        } catch(NoSuchFieldException nsfe) {
            throw new IOException("could not find usesDefaultInflater via reflection");
        } catch(IllegalAccessException iae) {
            throw new IOException("could not set usesDefaultInflater via reflection");
        }
        this.cb = cb;
     ...
     * @exception IOException if an I/O error has occurred
     */
    public GZIPInputStream(InputStream in, MemberOffset cb) throws Exception {
        this(new CountingInputStream(in), new Inflater(true), 1024 * 1024, cb);
    }
    ...
    public int read(byte[] buf, int off, int len) throws IOException {
        ...
        if (n == -1) {
            if (readTrailer()) {
                eos = true;
                cb.onMemberStart(streamIndex, cis.getCount());
            } else
     ...
     */
    private int readHeader(InputStream this_in) throws Exception {
        CheckedInputStream in = new CheckedInputStream(this_in, crc);
        crc.reset();
        // InflaterInputStream may have buffered data, so .getCount() doesn't
        ...
        // the footer is 8 bytes, but might be wrong.
        int start = streamIndex == 0 ? 0 : (cis.getCount() + (len <= 8 ? 0 : (-len + 8)));
        //System.out.println("start=" + start + ", count=" + cis.getCount() + ", off=" + off +", len=" + len + ", getTotalIn=" + inflater.getBytesRead());
        cb.onMemberStart(streamIndex, start);
        streamIndex++;
